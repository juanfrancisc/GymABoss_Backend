...
const listExercises = async (req, res, next) => {
    ...
    try {
        ...
        let consulta = 'SELECT e.title, e.id, e.typology, COUNT(l.id) AS n_like FROM user_like_exercises l RIGHT JOIN exercises e ON e.id=l.id_exercises GROUP BY e.id ORDER BY n_like DESC;'

...
const addLike = async (req, res, next) => {
    ...
    try {
        ...
        const [isLike] = await connection.query(
            `SELECT * FROM user_like_exercises WHERE id_user = ? AND id_exercises = ?`,
        ...
        if (isLike.length < 1) {
            ...
            const [likeCount] = await connection.query(
                `SELECT count(id) as likes FROM user_like_exercises WHERE id_exercises=?`,
                [idExercises]
            );
            ...
            await connection.query(
                `DELETE FROM user_like_exercises WHERE id_user = ? AND id_exercises = ?`,
                [idReqUser, idExercises]
            );
